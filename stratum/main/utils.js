...
exports.convertHashToBuffer = function (txs) {
    return txs.map((tx) => {
        if (tx.txid !== undefined)
            return exports.uint256BufferFromHash(tx.txid);
...
exports.decodeAddress = function (address, network) {
    ...
    try {
        ...
        if (decoded.prefix !== (network.bech32 || "bc"))
            throw new Error(
                `The address (${address}) given has an invalid prefix`,
            );
        if (decoded) {
...
exports.decodeBase58Address = function (address) {
    ...
    const version =
        payload.length === 22 ? payload.readUInt16BE(0) : payload[0];
...
exports.encodeAddress = function (address, type) {
    switch (type) {
        ...
        case "witnesspubkey":
            return exports.encodeChunks([
                exports.getBitcoinOPCodes("OP_0"),
                address,
            ]);
        case "witnessscript":
            return exports.encodeChunks([
...
exports.encodeChunks = function (chunks) {
    ...
    const bufferSize = chunks.reduce((accum, chunk) => {
        if (Buffer.isBuffer(chunk)) {
            ...
            return (
                accum + exports.getEncodingLength(chunk.length) + chunk.length
            );
        }
        return accum + 1;
    }, 0.0);
